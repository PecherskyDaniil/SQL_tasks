...
Попытка деления на ноль: тонн 150, часов 0. Возвращен 0.
test_zero|
---------+
        0|
...
Создайте функцию validate_sensor_reading(p_sensor_type VARCHAR, p_value NUMERIC):
...
Протестируйте все ветки (корректные данные и ошибки для каждого типа)
### Решение
```
...
SQL Error [S0002]: ERROR: Значение 120 вне диапазона для типа Скорость
  Подсказка: Допустимый диапазон: 0..50 км/ч
  Где: PL/pgSQL function validate_sensor_reading(character varying,numeric) line 18 at RAISE

SQL Error [S0001]: ERROR: Неизвестный тип датчика: Влажность
  Где: PL/pgSQL function validate_sensor_reading(character varying,numeric) line 13 at RAISE
...
Создайте анонимный блок, который вставляет 10 записей в fact_equipment_downtime
Среди записей намеренно включите ошибочные:
...
Каждая вставка должна быть в своём подблоке BEGIN ... EXCEPTION ... END
При ошибке:
Запишите ошибку в error_log через log_error()
Выведите RAISE WARNING с номером записи и типом ошибки
В конце выведите статистику: сколько вставлено, сколько ошибок
### Решение
```
DO $$ 
DECLARE
    ...
    -- Переменные для перехвата деталей ошибки
    v_msg TEXT; v_state TEXT; v_ctx TEXT;
BEGIN
    RAISE NOTICE '=== Пакетная загрузка: Полное заполнение fact_equipment_downtime ===';

    FOR i IN 1..10 LOOP
        BEGIN
            -- Имитация данных для каждого шага
            CASE i
                WHEN 3 THEN 
                    -- ОШИБКА 1: Несуществующий equipment_id (FK violation)
                ...
                WHEN 5 THEN 
                    -- ОШИБКА 2: NULL в NOT NULL поле (end_time)
                    INSERT INTO fact_equipment_downtime (
                        date_id, shift_id, equipment_id, reason_id, operator_id, 
                        location_id, start_time, end_time, duration_min, 
                        is_planned, comment, loaded_at
                ...
                ELSE
                    -- УСПЕШНАЯ ЗАПИСЬ (все столбцы заполнены)
                    INSERT INTO fact_equipment_downtime (
                        date_id, shift_id, equipment_id, reason_id, operator_id, 
                        location_id, start_time, end_time, duration_min, 
                        is_planned, comment, loaded_at
...
### Результат
```
=== Пакетная загрузка: Полное заполнение fact_equipment_downtime ===
ИТОГО: Успешно: 10, Ошибок: 0
```
## Задание 4
### Текст задания
...
Создайте функцию test_error_diagnostics(p_error_type INT):
...
В блоке EXCEPTION используйте GET STACKED DIAGNOSTICS для получения ВСЕХ доступных полей
Функция должна возвращать TABLE со столбцами:
field_name VARCHAR — имя диагностического поля
field_value TEXT — значение
Протестируйте для каждого типа ошибки и сравните, какие поля заполняются
### Решение
```
...
CONSTRAINT|                                                                                                 |
DATATYPE  |                                                                                                 |
CONTEXT   |SQL statement "SELECT 1 / 0"¶PL/pgSQL function test_error_diagnostics(integer) line 12 at PERFORM|
...
Создайте таблицу промежуточного хранения:
...
Вставьте тестовые данные (10 строк), включая ошибочные:
Несуществующая шахта
Некорректная дата ('32-01-2025')
Fe = 'N/A' (не число)
Fe = 150 (вне диапазона)
Корректные записи
Создайте функцию process_lab_import(), которая:
...
row_id|mine_name             |sample_date|fe_content|moisture|status|error_msg                                              |
------+----------------------+-----------+----------+--------+------+-------------------------------------------------------+
    21|Шахта "Северная"      |2025-01-10 |62.5      |5.2     |VALID |                                                       |
    22|Шахта "Несуществующая"|2025-01-11 |60.0      |4.0     |ERROR |Шахта "Шахта "Несуществующая"" не найдена в справочнике|
    23|Шахта "Западная"      |32-01-2025 |58.0      |3.5     |ERROR |Шахта "Шахта "Западная"" не найдена в справочнике      |
    24|Шахта "Южная"         |2025-01-12 |N/A       |4.1     |ERROR |Значение Fe не является числом: N/A                    |
    25|Шахта "Восточная"     |2025-01-13 |150       |2.0     |ERROR |Шахта "Шахта "Восточная"" не найдена в справочнике     |
    26|Шахта "Северная"      |2025-01-14 |61.2      |NULL    |VALID |                                                       |
    27|Шахта "Центральная"   |2025-01-15 |59.8      |4.8     |ERROR |Шахта "Шахта "Центральная"" не найдена в справочнике   |
    28|Шахта "Западная"      |2025-01-16 |-10       |5.0     |ERROR |Шахта "Шахта "Западная"" не найдена в справочнике      |
    29|Шахта "Восточная"     |2025-01-17 |63.0      |invalid |ERROR |Шахта "Шахта "Восточная"" не найдена в справочнике     |
    30|Шахта "Южная"         |2025-01-18 |55.5      |3.3     |VALID |                                                       |

log_id|log_time               |severity|source            |sqlstate|message                                               
------+-----------------------+--------+------------------+--------+------------------------------------------------------
    44|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Восточная"" не найдена в справочнике    
    43|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Западная"" не найдена в справочнике     
    42|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Центральная"" не найдена в справочнике  
    41|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Восточная"" не найдена в справочнике    
    40|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Значение Fe не является числом: N/A                   
    39|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Западная"" не найдена в справочнике     
    38|2026-04-04 12:11:43.945|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Несуществующая"" не найдена в справочник
    37|2026-04-04 12:11:28.353|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Восточная"" не найдена в справочнике    
    36|2026-04-04 12:11:28.353|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Западная"" не найдена в справочнике     
    35|2026-04-04 12:11:28.353|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Центральная"" не найдена в справочнике  
    34|2026-04-04 12:11:28.353|ОШИБКА  |process_lab_import|P0001   |Шахта "Шахта "Восточная"" не найдена в справочнике    
```
## Задание 6
### Текст задания
...
BEGIN
    ...
    -- Цикл по всем шахтам из справочника
    FOR v_mine IN SELECT mine_id, mine_name FROM dim_mine LOOP
        v_proc := v_proc + 1;
        
        BEGIN
            -- 1. Расчет показателей (используем подблоки для безопасности)
            -- Добыча
            SELECT COALESCE(SUM(tons_mined), 0) INTO v_tons 
            FROM fact_production WHERE mine_id = v_mine.mine_id AND date_id = p_date_id;

            -- OEE (Operating Hours / Planned Hours * 100)
            -- Предполагаем, что planned_hours = 24 часа на шахту в сутки
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Обработка ошибки для конкретной шахты
            v_err := v_err + 1;
            ...
            -- Логирование во внешнюю таблицу
            PERFORM log_error(
...
### Результат
```
mines_processed|mines_ok|mines_error|
---------------+--------+-----------+
              4|       0|          4|

date_id |mine_name       |tons_mined|status|error_detail                   |
--------+----------------+----------+------+-------------------------------+
20250115|Дубль           |          |ERROR |column "mine_id" does not exist|
20250115|Дубль           |          |ERROR |column "mine_id" does not exist|
20250115|Шахта "Северная"|          |ERROR |column "mine_id" does not exist|
20250115|Шахта "Южная"   |          |ERROR |column "mine_id" does not exist|
```